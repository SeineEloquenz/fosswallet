...
class PassParser(val context: Context? = null) {
    ...
    fun parse(
        ...
        if (!passJson.has("description")) {
            Log.w(TAG, context?.getString(R.string.no_description).toString())
            throw InvalidPassException()
        }
        val description = passJson.getString("description")
        val passVersion = passJson.optInt("formatVersion")
        if (passVersion != 1) {
            Log.w(TAG, context?.getString(R.string.invalid_format_version).toString())
            throw InvalidPassException()
        }
        if (!passJson.has("organizationName")) {
            Log.w(TAG, context?.getString(R.string.missing_organization_name).toString())
            throw InvalidPassException()
        }
        val organizationName = passJson.getString("organizationName")
        if (!passJson.has("serialNumber")) {
            Log.w(TAG, context?.getString(R.string.missing_serial_number).toString())
            throw InvalidPassException()
        }
    ...
    private fun parseRelevantDate(passJson: JSONObject): Long {
        return try {
            ...
            Log.w(TAG, context?.getString(R.string.failed_relevant_date).toString())
            0L
        }
    }
    ...
    private fun parseExpiration(passJson: JSONObject): Long {
        return try {
            ...
            Log.w(TAG, context?.getString(R.string.failed_expiration_date).toString())
            0L
        }
    }
    ...
    private fun parseBarcodes(passJson: JSONObject): Set<BarCode> {
        ...
        try {
            ...
            Log.e(TAG, context?.getString(R.string.exception_).toString())
        }
        return barcodes
    }
    ...
    private fun JSONObject.collectFields(name: String): List<PassField>? {
        return try {
            ...
            Log.e(TAG, context?.getString(R.string.fields_not_exist).toString())
            null
        }
    }
