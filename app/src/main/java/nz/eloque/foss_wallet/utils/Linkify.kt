...
fun linkifyUrls(text: String): String {
    return Regex(PatternsCompat.WEB_URL.pattern(), RegexOption.IGNORE_CASE).replace(text) {
...
fun linkifyMails(text: String): String {
    return Regex(PatternsCompat.EMAIL_ADDRESS.pattern(), RegexOption.IGNORE_CASE).replace(text) {
        if (it.value.contains("data-linkified")) return@replace it.value
