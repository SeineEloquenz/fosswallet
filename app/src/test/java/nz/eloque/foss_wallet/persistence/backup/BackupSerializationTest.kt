                ...
                secondaryFields =
                    listOf(
                        PassField(
                            "s1",
                            null,
                            PassContent.Date(PassDateTime.Floating(zoned.toLocalDateTime()), FormatStyle.MEDIUM, isRelative = false),
                        ),
                    ),
                auxiliaryFields =
                    listOf(
                        PassField("a1", "Aux", PassContent.Time(PassDateTime.Absolute(zoned), FormatStyle.SHORT, isRelative = true)),
                    ),
                backFields =
                    listOf(
                        PassField("b1", "Back", PassContent.DateTime(PassDateTime.Absolute(zoned), FormatStyle.LONG, isRelative = false)),
                    ),
            )
        assertEquals(pass, roundTrip(pass))
    }
